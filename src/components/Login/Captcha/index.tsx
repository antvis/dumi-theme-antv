...
const Captcha = forwardRef(({
  ...
  const ncInstance = useRef<CaptchaInstance | null>(null);
  useEffect(() => {
  ...
  function loadCaptcha() {
    ...
    const nc_appkey = window.appData?.captchaAppKey || 'FFFF000000000179A3AD';
    ...
    const lang = {
      ...
      _yesTEXT: en ? formatMessage({ id: 'login.captcha.success.en' }) : formatMessage({ id: 'login.captcha.success' }),
      _Loading: en ? formatMessage({ id: 'login.captcha.loading.en' }) : formatMessage({ id: 'login.captcha.loading' }),
      _error300: en ? 
        formatMessage({ id: 'login.captcha.error.wrong.en' }) + ' <a href="javascript:__nc.reset()">' + formatMessage({ id: 'login.captcha.error.refresh.en' }) + '</a>' : 
        formatMessage({ id: 'login.captcha.error.wrong' }) + ' <a href="javascript:__nc.reset()">' + formatMessage({ id: 'login.captcha.error.refresh' }) + '</a>',
      _errorNetwork: en ? 
        formatMessage({ id: 'login.captcha.error.network.en' }) + ' <a href="javascript:__nc.reset()">' + formatMessage({ id: 'login.captcha.error.refresh.en' }) + '</a>' : 
        formatMessage({ id: 'login.captcha.error.network' }) + ' <a href="javascript:__nc.reset()">' + formatMessage({ id: 'login.captcha.error.refresh' }) + '</a>',
    };
    if (en) {
    ...
    >
      ...
      <Form.Item
        ...
        rules={[
          {
            required: true,
            message: en ? formatMessage({ id: 'login.captcha.required.en' }) : formatMessage({ id: 'login.captcha.required' }),
          },
        ]}
